...
const AllEvents = () => {
    ...
    useEffect(() => {
        let businessId = window.location.pathname.split('/')[3];
        setBusinessId(window.location.pathname.split('/')[3]);
        axios(`/singlebusinessreviews/${businessId}`)
            .then(reviewsBox => {
    ...
    const deleteEventReview = e => {
        ...
        axios.post('/admin/deleteBusinessReview', {
            reviewId: e,
            businessId
        }, {
            headers: {
                                                ...
                                                !errorMessage ?
                                                    <div className="category_display grid_3">
                                                        {reviewsBox.map((reviews, index) => {
                                                            return (
                                                                <div key={index}>
                                                                    <Rate disabled allowHalf defaultValue={+reviews.star} />
                                                                    <p
