...
const AllEvents = () => {
    ...
    useEffect(() => {
        ...
        setCommunityId(window.location.pathname.split('/')[3]);
        axios(`/singlecommunityreviews/${communityId}`)
            .then(reviewsBox => {
    ...
    const deleteCommunityReview = e => {
        setErrorMessage('');
        setSpinnerStatus(true);
        axios.post('/admin/deleteCommunityReview', {
            reviewId: e,
            communityId
        }, {
            headers: {
                                                        ...
                                                        {reviewsBox.map((reviews, index) => {
                                                            return (
                                                                <div key={index}>
                                                                    ...
                                                                    <button
                                                                        onClick={() => deleteCommunityReview(reviews.id)}
                                                                        style={{ padding: '10px' }}
