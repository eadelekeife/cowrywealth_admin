...
const AllEvents = () => {
    ...
    const data = statesData.map((state, index) => {
        return {
            sn: index + 1,
            key: index,
            state: `${state.name}`,
            lga: state.LocalGovernments.length,
            eventsAdded: state.EventsRecords.length,
            communitiesAdded: state.CommunityRecords.length,
            businessesAdded: state.BusinessRecords.length,
        ...
        axios.post(url, {
            ...
            .then(categoryData => {
                if (categoryData.data.summary === "success") {
                    setStatesData(categoryData.data.message);
                    setSpinnerStatus(false);
                } else {
